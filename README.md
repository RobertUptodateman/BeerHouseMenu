...
- Выглядит как меню: строка «номер крана, цветной маркер, название, стиль — ABV, IBU, цена».
- Без названия бара вверху. Светлый фон, палитра по логотипу (`small_logo.jpg`): зелёный хмеля `#18634A` для цен и заголовков колонок, неоново-жёлтый `#E8EC94` для плашек Core/Guest. Цвета — переменные в начале `<style>` в `index.html`.
...
- Названия крупные, ABV и IBU мельче. Стили пишутся по-английски, по-русски — только некоторые названия.
- Цена в лари (₾). Объём указывается рядом с ценой только для бокала 0,33; остальное льётся в стандартный 0,45.
- Маркер цвета пива: светлое, янтарное/красное, тёмное, фруктовое/кислое.
...
- Путь: `/admin/`.
- Decap CMS.
- Редактирование одного JSON-файла: два списка, «Базовые краны» и «Гостевые краны». Позиции добавляются, удаляются и переставляются перетаскиванием.
- У каждой позиции в списке виден номер крана («Кран 1» … «Кран N»); после перетаскивания номера обновляются сразу. Номера рисуются CSS-счётчиком в `admin/index.html` и опираются на метки в классах Decap (`SortableListItem`, `TopBar`); при смене версии Decap их стоит проверить.
...
`data/menu.json` — два списка: `core` (базовые краны, левая колонка) и `guest` (гостевые, правая). Номер крана не хранится, а считается по порядку: `core` — краны 1…N, `guest` — продолжение нумерации.
...
| `color` | маркер: `light`, `amber`, `dark`, `fruit` |
| `glass` | бокал: `0.45` (стандарт, не показывается) или `0.33` |
| `price` | цена в лари |
...
- **Формат JSON.** Decap `files` пишет объект верхнего уровня, поэтому списки лежат в полях `core` и `guest`. Страница также понимает старые форматы (`beers` и одна позиция) и показывает их как базовые краны.
...
- **Номера кранов в админке.** Если у одного из списков свернуть весь список целиком (стрелка у заголовка «Базовые краны»), нумерация гостевых в админке может сбиться. На сайте номера всегда правильные.
